...
async def test_pdf(nua_config: AsyncNuaClient):
    path = define_path("2310.14587.pdf")
    nc = AsyncNucliaProcessing()
    payload = await nc.process_file(path, kbid="kbid", timeout=300, nc=nua_config)
    assert payload
    assert (
...
async def test_video(nua_config: AsyncNuaClient):
...
@pytest.mark.asyncio_cooperative
@pytest.mark.timeout(660)
async def test_vude_1(nua_config: AsyncNuaClient):
    path = define_path(
...
@pytest.mark.asyncio_cooperative
async def test_activity(nua_config: AsyncNuaClient):
    nc = AsyncNucliaProcessing()
    await nc.status(nc=nua_config)
