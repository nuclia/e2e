...
1 - Create a new `pre-existing-kb` in the new zone, on the `automated-testing` account. Name the kb `{PROVIDER} {REGION} pre-existing-kb` to easily distinguish in the dashboard, but keep the slugs the same.
