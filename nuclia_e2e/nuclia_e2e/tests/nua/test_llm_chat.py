...
async def test_llm_chat(nua_client: AsyncNuaClient):
    ...
    chat_model = ChatModel(
        ...
        system=(
            "You are a helpful assistant, your first word is always the language you will "
            "be using in the conversation in all caps.\nExample: 'FRANÇAIS: Bonjour, madame.'"
        ),
        user_prompt=UserPrompt(
            prompt=(
                "Respond to the question using the context pieces provided, please answer "
                "in ITALIAN:\n Question: {question}\n Context: {context}"
            )
        ),
        query_context={
    ...
    assert "1" in generated.citations
    assert "2" not in generated.citations
