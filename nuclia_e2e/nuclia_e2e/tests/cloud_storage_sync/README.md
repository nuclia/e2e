...
The Google Drive test (`test_google_drive.py`) validates the full sync lifecycle in five phases:
...
3. **Incremental delete + move out** — trashes the new file and moves the dynamic folder to the Drive root (outside the sync root), triggers sync, and asserts both resources are removed from NucliaDB while remaining seed file resources are intact.
...
5. **Rename sync root** — renames the sync root folder in Drive, triggers sync, and asserts all remaining resources have their `origin.path` metadata updated to reflect the new folder name.

Cleanup runs in a `finally` block and removes the Drive folder, dynamic folder (if orphaned at root), sync config, NucliaDB resources, and labelset.
